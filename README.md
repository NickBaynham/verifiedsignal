...
- **CI** — **[`.github/workflows/ci.yml`](.github/workflows/ci.yml)** — **Python** job: Ruff on `src`, `tests`, `app`, `worker`; Postgres **16** + migrations; **`pdm run python -m pytest`** (full suite). **Web** job: **`apps/web`** — `npm ci`, Playwright Chromium, **`npm run build`**, **`test:unit`**, **`test:e2e`**, **`test:e2e:api-mock`**.
...
**Still stubbed / later stages:** **LLM**-based scoring (today: **heuristic** canonical **`document_scores`** from the pipeline plus optional non-canonical **`score_document`** stub row), **vector** search, and durable cross-instance SSE (today’s **`EventHub`** is in-process; the SPA also **polls** **`GET /api/v1/documents/{id}/pipeline`** for worker progress). **Keyword search** uses **text extract** (PDF/DOCX/plain) + **OpenSearch** (or **`USE_FAKE_OPENSEARCH`**). Pipeline persistence: **`pipeline_runs`** / **`pipeline_events`**; optional **`ENQUEUE_SCORE_AFTER_PIPELINE`** queues **`score_document`**.
