...
- **Supabase session auth (optional)** — FastAPI routes under **`/auth`** (signup, login with **httpOnly** refresh cookie on path `/auth`, refresh, logout, reset email); JWT validation via **JWKS** or **HS256** (`app/auth/`). Protected examples: **`/api/v1/documents`**, **`/api/v1/collections`**, **`/api/v1/users/me`**. See **[`docs/auth-supabase.md`](docs/auth-supabase.md)**, **[`supabase/README.md`](supabase/README.md)**, and **`apps/web/README.md`** for the React/Vite side.
