...
- **CI** — `**[.github/workflows/ci.yml](.github/workflows/ci.yml)`** — **Python** job: Ruff on `src`, `tests`, `app`, `worker`, `**scripts**`, `**mcp_server**`; `**pip-audit**` on exported requirements; Postgres **16** + migrations; `**pytest`** with `**--cov=app/services**` (line coverage floor **45%** via `**pyproject.toml`** `[tool.coverage.report]`, `**term-missing**` + `**coverage.xml**` artifact). **Web** job: `**apps/web`** — `npm ci`, `**npm audit**`, Playwright Chromium, `**npm run build**`, `**test:unit**`, `**test:e2e**`, `**test:e2e:api-mock**`. **Dependabot** — `**[.github/dependabot.yml](.github/dependabot.yml)`** for **GitHub Actions**, **pip** (root / PDM lock), and `**apps/web`** npm.
...
The stack is defined in `docker-compose.yml`. `**make docker-up**` (or `docker compose up --build`) starts every service below except `**test**`, which uses the `**test**` profile.

**Scoring:** the **scoring-service** image is built from the sibling directory `**../scoring-service**` (override `**SCORING_SERVICE_REPO**` in `.env`). It uses its own **scoring-postgres** volume. Set **`OPENAI_API_KEY`** in `.env` for real LLM scores. The VerifiedSignal **worker** defaults to **`SCORE_HTTP_URL=http://scoring-service:8000/v1/score`** and **`SCORE_HTTP_BEARER_TOKEN=changeme-scoring-bearer`** unless you override them (keep the same token as **`VERIFIEDSIGNAL_HTTP_BEARER_TOKEN`** inside scoring-service; Compose wires this from **`SCORE_HTTP_BEARER_TOKEN`**).
...
| **scoring-postgres**      | Postgres for **scoring-service** only (internal; no host port)                       | —                              |
| **scoring-service**       | LLM scorer API (**`/v1/score`** for VerifiedSignal worker)                            | `**8010**` → `SCORING_SERVICE_PORT` |
| **app**                   | **FastAPI** (`pdm run api-prod` → uvicorn `app.main:app`); health **`/api/v1/health`** | `**8000`** → `API_PORT`        |
| **worker**                | **ARQ** consumer (`pdm run worker`)                                                   | —                              |
| **web**                   | Static **React** build (nginx); **`VITE_API_URL`** baked at image build (`**WEB_VITE_API_URL**`, default `http://127.0.0.1:8000`) | `**5173**` → `WEB_PORT`        |
...
**Option A — static UI in Compose:** the **`web`** service serves the production build at **`http://127.0.0.1:5173`** (override **`WEB_PORT`**). Rebuild after UI changes: **`docker compose up -d --build web`**. If **`API_PORT`** is not `8000`, set **`WEB_VITE_API_URL=http://127.0.0.1:<port>`** so the SPA calls the correct API origin.

**Option B — Vite on the host:** the React app lives in `**apps/web**`. To use **API mode** (real backend instead of mock demo data):
